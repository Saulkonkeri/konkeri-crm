...
export default function CRMPage() {
  ...
  // --- LISTAS DINÁMICAS PARA LOS SELECTS DE FILTRO ---
  ...
  const handleDrop = async (e: React.DragEvent, nuevoEstado: string) => {
    ...
    try {
      ...
      alert("Hubo un error al mover el cliente. Se recargarán los datos.");
      cargarClientes();
    }
  };
  ...
  const guardarNuevoCliente = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload: any = {
        ...
        email: nuevoEmail ? nuevoEmail.trim().toLowerCase() : null, 
        ciudad_residencia: nuevaCiudad.trim() || null, 
        motivo_compra: nuevoMotivo, 
        tipologia_interes: nuevoInteres, 
        origen_captacion: nuevoOrigen, 
        notas: notaInicial, 
        estado: 'Interesado', 
        tipo: 'prospecto', 
        temperatura: '❄️ Frío'
      ...
      if (nuevoIngresadoPor.trim()) {
        payload.ingresado_por = nuevoIngresadoPor.trim();
      }
      ...
      if (data && data.length > 0) {
        ...
        setNuevoIngresadoPor('Saúl Intriago / Debbi Mera');
        setNuevoOrigen('Meta Ads'); setNuevoMotivo('Para Invertir'); setNuevoInteres('Suite');
      }
    } catch (error: any) {
  ...
  const guardarProximaTarea = async () => {
    ...
    try {
      const updates = {
        proximo_contacto: fechaAccion || null,
        tipo_accion: tipoAccion,
        detalle_accion: detalleAccion || null
      };
  ...
  const abrirWhatsApp = (cliente: Cliente, tipoMensaje: 'bienvenida' | 'campana' | 'libre') => {
    ...
    if (tipoMensaje === 'bienvenida') {
      const msg = plantillaMensaje.replace('{nombre}', cliente.nombres);
      txt = `?text=${encodeURIComponent(msg)}`;
    } else if (tipoMensaje === 'campana') {
      const msg = plantillaCampana.replace('{nombre}', cliente.nombres);
      txt = `?text=${encodeURIComponent(msg)}`;
    }
  ...
  // --- FILTRADO INTELIGENTE (BÚSQUEDA + CIUDAD + TIPOLOGÍA + ORIGEN) ---
  const clientesFiltrados = useMemo(() => {
    return clientes.filter(c => {
      ...
      const coincideBusqueda = 
        ...
        (c.notas && c.notas.toLowerCase().includes(b));
  ...
  return (
    <div className="min-h-screen bg-[#F4F4F4] px-4 md:px-6 py-6 font-sans text-neutral-800 flex flex-col h-screen overflow-hidden">
      ...
      <div className="w-full flex-shrink-0 mb-4 space-y-3">
        ...
        <div className="bg-white p-3 rounded-xl border border-neutral-200 shadow-sm flex flex-wrap items-center gap-3">
          <div className="relative flex-1 min-w-[220px]">
            ...
            <input 
              type="text" 
              placeholder="Buscar por nombre, teléfono o notas (ej. autos)..." 
              value={busqueda} 
          ...
          <div className="flex items-center gap-1.5 text-xs">
            ...
            <select 
              ...
              className="bg-neutral-50 border border-neutral-200 rounded-lg py-2 px-2.5 text-xs font-semibold text-neutral-700 outline-none focus:border-[#B94A36]"
            >
          ...
          <div className="flex items-center gap-1.5 text-xs">
            <span className="text-neutral-400 font-medium text-[10px] uppercase">Interés:</span>
            <select 
            ...
            >
              <option value="Todas">Todas</option>
              {tipologiasDisponibles.map(t => <option key={t} value={t}>{t}</option>)}
            </select>
          </div>
      ...
      <div className="w-full flex-1 min-h-0 overflow-hidden relative">
        ...
        {/* KANBAN (Grid de 7 columnas exactas) */}
        {vista === 'kanban' && (
            ...
            {estados.map(estado => {
              ...
              return (
                <div 
                  ...
                  className="bg-neutral-200/40 rounded-xl p-2 flex flex-col h-full overflow-hidden border border-neutral-200/60"
                >
                  ...
                  <div className="space-y-2 overflow-y-auto flex-1 pr-1 custom-scrollbar">
                    {leads.map(cliente => (
                      <div 
                        ...
                        className={`bg-white p-2.5 rounded-lg shadow-sm border ${cliente.tipo === 'cliente' ? 'border-[#B94A36]/40' : 'border-neutral-200'} hover:border-[#B94A36] cursor-pointer transition-all relative group cursor-grab active:cursor-grabbing`}
                      >
                        {cliente.temperatura && <span className="absolute top-2 right-2 text-[10px]" title={cliente.temperatura}>{cliente.temperatura.split(' ')[0]}</span>}
                        
                        {/* Etiqueta Visual de Inversionista */}
                        {cliente.tipo === 'cliente' && (
                          <div className="mb-1">
                            <span className="bg-[#B94A36]/10 text-[#B94A36] text-[7px] font-bold px-1.5 py-0.5 rounded uppercase">👑 Inversionista</span>
                          </div>
                        )}

                        <h4 className="font-bold text-neutral-900 text-[11px] pr-4 leading-tight">{cliente.nombres} {cliente.apellidos}</h4>
                        <p className="text-[9px] text-neutral-500 font-mono mt-0.5 truncate">{cliente.telefono || 'Sin celular'}</p>
                        
                        {cliente.ciudad_residencia && (
                          <span className="inline-block mt-1 bg-neutral-100 text-neutral-600 text-[8px] font-semibold px-1.5 py-0.2 rounded">
                            📍 {cliente.ciudad_residencia}
                          </span>
                        )}
                        
                        {cliente.proximo_contacto && (
                          <div className={`mt-1.5 text-[8px] font-bold px-1.5 py-0.5 inline-block rounded border ${esFechaVencida(cliente.proximo_contacto) ? 'bg-red-50 text-red-700 border-red-200' : 'bg-blue-50 text-blue-700 border-blue-200'}`}>
                            📅 {new Date(cliente.proximo_contacto).toLocaleDateString('es-EC', {day:'2-digit', month:'short'})}
                            {cliente.tipo_accion && ` - ${cliente.tipo_accion.split(' ')[0]}`}
                          </div>
                        )}
                      </div>
                    ))}
            ...
            <table className="w-full text-left text-sm border-collapse min-w-[800px]">
              <thead className="sticky top-0 bg-neutral-900 z-10">
                <tr className="text-white text-[10px] uppercase tracking-wider">
                  ...
                  <th className="px-4 py-3 font-semibold">Última Nota</th>
                </tr>
              </thead>
                  ...
                  <tr key={cliente.id} onClick={() => setClienteSeleccionado(cliente)} className="hover:bg-neutral-50 cursor-pointer">
                    <td className="px-4 py-3">
                      <div className="font-bold text-neutral-900 text-xs">
                        {cliente.tipo === 'cliente' && <span className="mr-1" title="Inversionista">👑</span>}
                        {cliente.nombres} {cliente.apellidos}
                      </div>
                      <div className="text-[10px] text-neutral-400 font-mono">{cliente.telefono}</div>
                    </td>
                    <td className="px-4 py-3 text-xs text-neutral-600 font-medium">
                      {cliente.ciudad_residencia || '---'}
                    </td>
                    <td className="px-4 py-3">
                      <div className="text-[10px] font-bold text-neutral-700 bg-neutral-100 inline-block px-1.5 py-0.5 rounded mb-0.5">{cliente.estado}</div>
                    </td>
                    <td className="px-4 py-3 text-xs font-medium text-neutral-800">
                      {cliente.ingresado_por || '---'}
                    </td>
                    <td className="px-4 py-3">
                      {cliente.proximo_contacto ? (
                         <div>
                           <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${esFechaVencida(cliente.proximo_contacto) ? 'bg-red-100 text-red-700' : 'bg-blue-50 text-blue-700'}`}>
                             {new Date(cliente.proximo_contacto).toLocaleDateString('es-EC')} - {cliente.tipo_accion}
                           </span>
                           <p className="text-[10px] text-neutral-500 mt-1 truncate max-w-[200px]">{cliente.detalle_accion}</p>
                         </div>
                      ) : <span className="text-[10px] text-neutral-400">Sin agendar</span>}
                    </td>
                    <td className="px-4 py-3 text-xs text-neutral-500 truncate max-w-[200px]">
                      {cliente.notas?.split('\n')[0] || '-'}
                    </td>
                  </tr>
                ))}
          ...
          <>
            <div className="p-5 border-b border-neutral-100 bg-neutral-50 relative flex-shrink-0">
              ...
              <div className="flex flex-wrap items-center gap-1.5 mt-2">
                ...
                {clienteSeleccionado.ingresado_por && (
                  <span className="bg-[#B94A36]/10 text-[#B94A36] text-[9px] font-bold px-1.5 py-0.5 rounded uppercase">Por: {clienteSeleccionado.ingresado_por}</span>
                )}
              </div>
            </div>
