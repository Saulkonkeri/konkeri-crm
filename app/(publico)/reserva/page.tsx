...
export default function ReservaExpressPage() {
  ...
  // INICIAMOS CON EL PLAN B PARA QUE NUNCA SE VEA VACÍO
      ...
      <main className="max-w-5xl mx-auto px-3 md:px-4 mt-6">
          ...
          <div className="min-h-[80vh] flex flex-col items-center justify-center animate-in fade-in duration-700 px-4">
            <div className="bg-white p-8 md:p-12 rounded-3xl border border-[#EAE3DC] shadow-xl max-w-md w-full text-center relative overflow-hidden">
              ...
              <form onSubmit={verificarAcceso} className="space-y-4">
                <div>
                  <input type="email" required value={emailAcceso} onChange={(e) => setEmailAcceso(e.target.value)} placeholder="tucorreo@ejemplo.com" className="w-full bg-neutral-50 border border-neutral-200 rounded-xl p-4 text-base text-center focus:outline-none focus:border-[#B94A36] focus:ring-1 focus:ring-[#B94A36] transition-all" />
                </div>
                {errorAcceso && <p className="text-xs text-red-500">{errorAcceso}</p>}
                <button type="submit" disabled={cargandoAcceso} className="w-full bg-neutral-900 text-white font-bold uppercase tracking-widest text-xs py-4 rounded-xl hover:bg-black transition-all shadow-lg shadow-neutral-900/20 disabled:opacity-70 flex justify-center items-center">
        ...
        {paso === 'filtro' && (
          <div className="max-w-lg mx-auto text-center space-y-8 animate-in zoom-in-95 duration-500 mt-10 md:mt-20">
            <div>
              <h2 className="text-3xl font-light text-neutral-900 mb-2 px-2">Selecciona la tipología.</h2>
              <p className="text-sm text-neutral-500 px-4">Indícanos qué distribución se adapta a tu requerimiento.</p>
            </div>
        ...
        {paso === 'mapa' && (
          <div className="space-y-4 md:space-y-6 animate-in slide-in-from-bottom-8 duration-500 w-full max-w-4xl mx-auto">
            <div className="flex flex-col md:flex-row md:justify-between md:items-end px-1 gap-4">
              <div>
                <button onClick={() => setPaso('filtro')} className="text-[10px] font-bold text-neutral-400 uppercase tracking-wider mb-2 hover:text-[#B94A36] flex items-center gap-1">← Volver</button>
          ...
          <div className="max-w-md mx-auto bg-white p-6 md:p-8 rounded-2xl border border-[#EAE3DC] shadow-sm animate-in slide-in-from-right-8 mt-10">
            ...
            <form onSubmit={procesarReserva} className="space-y-4">
              <div>
                ...
                <input required type="text" value={formData.nombres} onChange={e => setFormData({...formData, nombres: e.target.value})} className="w-full bg-neutral-50 border border-neutral-200 rounded-lg p-3 text-base focus:outline-none focus:border-[#B94A36]" />
              </div>
              <div>
                <label className="block text-[10px] font-bold text-neutral-500 uppercase mb-1">Cédula / Pasaporte</label>
                <input required type="text" value={formData.cedula} onChange={e => setFormData({...formData, cedula: e.target.value})} className="w-full bg-neutral-50 border border-neutral-200 rounded-lg p-3 text-base focus:outline-none focus:border-[#B94A36]" />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-bold text-neutral-500 uppercase mb-1">WhatsApp</label>
                  <input required type="tel" value={formData.telefono} onChange={e => setFormData({...formData, telefono: e.target.value})} className="w-full bg-neutral-50 border border-neutral-200 rounded-lg p-3 text-base focus:outline-none focus:border-[#B94A36]" />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-neutral-500 uppercase mb-1">Correo</label>
        ...
        {paso === 'exito' && (
          <div className="max-w-md mx-auto bg-white p-8 rounded-2xl border-2 border-emerald-500 shadow-xl text-center animate-in zoom-in-95 duration-500 mb-10 mt-10">
            ...
            <p className="text-sm text-neutral-600 mb-6">La solicitud para la <strong>Unidad {unidadSeleccionada.id}</strong> ha sido enviada con éxito.</p>
            <div className="mb-6">
              <button onClick={enviarNotificacionReservaWhatsApp} className="w-full bg-[#25D366] hover:bg-[#1DA851] text-white font-bold uppercase tracking-wider text-xs py-3.5 rounded-xl transition-all shadow-md">
                📲 Notificar a Asesores
              </button>
            </div>
            <button onClick={() => window.location.reload()} className="mt-8 text-xs font-bold text-neutral-400 hover:text-neutral-800 uppercase underline">Finalizar y salir</button>
          </div>
        )}
      </main>
    </div>
